...
import { convertToResidence } from "../../utils/formConverter";
import { useNavigate } from "react-router-dom";

function AddResidence() {
  const { register, handleSubmit } = useForm<ResidenceForm>();
  ...
  const onSubmit: SubmitHandler<ResidenceForm> = (data) => {
    console.log(data);
    setLoading(true);
    Api.postResidence(convertToResidence("1", data)).then(() => {
      setLoading(false);
      navigate("/residences")
    });
  };
  return loading ? (
    ...
    <div className="flex flex-col items-center ">
      <h1 className="pt-8 pb-8 text-3xl font-bold">Add your residence</h1>
      <form
